...
app.use(/\/(?!api\/v1\/auth).*/, jwtMiddleware);
...
app.use((err, req, res, next) => {
  if (err.constructor.name === 'SendGridError') {
    const body = JSON.parse(err.response.body);
    const message = body.errors.map(sendgridError => sendgridError.message).join('\n');
    const status = err.response.statusCode || httpStatus.INTERNAL_SERVER_ERROR;

    const apiError = new APIError(message, status, true);
...
app.use((err, req, res, next) =>		// eslint-disable-line no-unused-vars
  res.status(err.status).json({
    status: `${err.status} ${httpStatus[err.status]}`,
